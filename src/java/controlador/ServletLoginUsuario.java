...
public class ServletLoginUsuario extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean respuesta = UsuarioDAO.validarPassword(nombreU, password);
        if (respuesta) {
            //cambiar jsp por el registro exitoso
            request.getSession().setAttribute("nombreU", nombreU);
            response.sendRedirect("plantillas/inicio.jsp");
            
            //request.getRequestDispatcher("plantillas/inicio.jsp").forward(request, response);
        } else {
